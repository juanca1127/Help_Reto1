...
const ModalFecha = ({ show, handleClose }) => {
  ...
  const env = async () => {
    const data1 = await fetch(`${process.env.REACT_APP_SERVER_URL}/fechas`, {
      method: "POST",
      body: JSON.stringify({
    ...
    <>
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Ventas segun su periodo</Modal.Title>
        </Modal.Header>
        <Modal.Body>
      ...
      >
        <Modal.Header closeButton>
           <small>Mas vendido desde {format(startDate)} hasta {format(startDate1)}</small>
        </Modal.Header>
        <Modal.Body>
